...
**更新於 2026-07-19 · 下次更新 2026-07-26（每週日）**
**待 Michael 定稿。不自動發布、不自動改網站、不自動群發。**

## 這次套用了兩條新鐵律（Michael 07-19 訂的,以後每週都套用）

1. **不收台灣案例。** 只做 SG／HK／UK 三區,原本篩到的 TSMC 直接拿掉,不放。
2. **薪資只放 high confidence 數字,查無就留白。** 本週 9 個裡,只有 **2 個**薪資達標:Waymo(職缺自己揭露)、法興銀行(該公司該地區有具名、互相印證的薪資數據)。其餘 7 個原本我推估過區間,但來源其實是「新加坡 AVP 平均」「香港 MD 投行平均」這種泛用職級城市均值,套到具體職缺上等於瞎猜,這次全部拿掉,誠實寫「薪資未揭露」。如果你覺得達標數太少,下週可以多看幾個公司官網 careers 頁,那邊比較常見直接寫級距。

另外也補了兩條規則到 `C:\Users\michael\.claude\scheduled-tasks\green-jobs-weekly\SKILL.md`,以後每週自動套用：只收 posted 14 天內的職缺、定稿前每個連結都實際打開測試過(本次 9 個全部重新測試,確認都還在收件,沒有 404 或已關閉)。

## 一句話總結

本週 **9 個**精選,新加坡最熱(4 個),香港、英國各 2 到 3 個。濾掉約 12 個雜訊：MBA 關鍵字帶出的通用管理職(HSBC 台灣、Taiwan Mobile 投資職、American Express、SG Growth Capital——查了 JD 後這幾個雖然沾到永續或影響力字眼,但核心職務是通用投資或管理,不是真的以永續／影響力為主軸)、一個已下架(IIX Associate Director,LinkedIn 顯示「不再接受應徵」)、一個跟上週精選重複(MSCI EMEA 永續與氣候指數主管)、一個台灣案例(TSMC,依新鐵律不收)。

---

## 待貼進 `content/greenJobs.ts` 的 `weeklyPicks`（9 筆）
...
## LINE 群發草稿（待 Michael 按送出）

本週亞太綠領情報,幫你抓了幾個有意思的訊號。

BlackRock 跟淡馬錫合資的減碳投資平台在新加坡找分析師,做的是儲能電網這種真金白銀的成長股權投資。

Waymo 在倫敦開的永續合規職,薪水直接寫在職缺上,88000到92500英鎊,這在自駕車業界很少見。

法興銀行在香港找副總裁,做中國資金投向電池礦業的結構融資,薪資估 HK$100萬到150萬。

完整週報,含南極碳匯、Berge Bulk、Airwallex 這幾個的詳細點評在這:{綠領職缺頁連結}?utm_source=line&utm_medium=broadcast

---

**提醒 Michael:**
1. 改完點評／確認薪資 → 說一聲我 bump `updatedAt` 並部署。
2. 這次套用了兩條新鐵律:不收台灣、薪資只放 high confidence,已經寫進排程任務的 SKILL.md,以後每週自動套用,不用再交代第二次。
3. 9 個裡有 7 個薪資留白,是套用新規則後誠實的結果,不是偷懶——原本的推估來源都是泛用職級城市均值,不夠格。
4. 9 個原始職缺連結,今天(7/19)都重新打開測試過,確認還在收件、沒有 404 或已關閉。
5. LINE 群發吃 200 則/月免費額度,好友數變多時要留意。
