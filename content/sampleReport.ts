...
export const sampleReports: Record<Locale, SampleReport> = {
  'zh-TW': {
    ...
    sections: {
      ...
      core_story_gap: {
        body: '這份材料最大的故事缺口，是裡面沒有出現一個屬於你自己的氣候觀點。知識的部分很完整：6 年經驗、GRI 與 SASB、SBTi 與 CBAM 都在。但從第一頁到最後一頁，寫的都是你參與過哪些專案、導入過哪些框架，沒有一句在回答「台灣製造業的氣候轉型，真正的障礙在哪裡」這種需要立場的題目。機制是這樣：招募者評估資深候選人的時候，讀的是「從這些工作長出了什麼看法」，而你自己說的「說不太清楚下一步要往哪走」，在他們耳裡就是這個人還沒把經驗轉成觀點。這一點在氣候金融的面試裡會被追到底，因為那裡的第一題幾乎都是「這筆風險你當時怎麼定價」，而一份框架清單回答不了那一題。',
      ...
      mba_readiness: {
        body: '這一步的關卡在「申請理由」還沒成形，跟資歷深度無關。你自己寫的是「可能 6 到 12 個月內申請，但還沒決定要不要」，而 MBA 論文要回答的正是「我要去哪、為什麼 MBA 是必要的那一步」。這兩件事現在對不上，所以照這個狀態寫出來會是一篇方向模糊的論文。而方向模糊在申請裡的代價很具體：同一份資歷，賣掉的會是低一個層級的學校。',
      ...
      international_positioning: {
        body: '你的輪廓目前偏台灣在地。CBAM 與 TCFD 是國際框架，是你少數帶得走、跨市場都讀得懂的訊號，但你的經驗敘述仍以台灣客戶為主，跨區可信度尚未被點亮。',
      ...
      cv_readiness: {
        body: '你 CV 最弱的一處結構，是「用框架名稱代替成果」：每一條都是做了什麼框架，少有量化結果或你做的判斷。讀者看完知道你接觸過什麼，但不知道你因此改變了什麼。',
